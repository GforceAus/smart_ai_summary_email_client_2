...
help:
    ...
    @echo "  ./deploy.sh run                      Trigger full run now (sends emails)"
    @echo "  ./deploy.sh run-dry                  Trigger dry run now (no emails)"
    @echo "  ./deploy.sh status                   Container + timer health"
    @echo "  ./deploy.sh logs                     Last 100 log lines"
    @echo "  ./deploy.sh schedule [HH:MM]         Install Monday timer (default 07:00)"
    @echo "  ./deploy.sh change-time HH:MM        Change scheduled time"
    @echo "  ./deploy.sh next-run                 Show when next run fires"
    @echo ""
    @echo "  First deploy:"
    @echo "    ./deploy.sh sync"
