...
help:
    ...
    @echo ""
    @echo "── Deploy (run ON the server) ────────────────────────────"
    @echo "  ./deploy.sh build                        Rebuild Docker image"
    @echo "  ./deploy.sh update                       git pull + rebuild"
    ...
    @echo "  ./deploy.sh check                        Show what would run today"
    ...
    @echo ""
    @echo "  First deploy (on server):"
    @echo "    git clone <repo> && cd smart_ai_summary_email_client_2"
    @echo "    scp local:.env .env"
